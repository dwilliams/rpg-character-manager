...
class TestADNDItemCreation(unittest.TestCase):
    ...
    def test_create_adnd(self):
        ...
        item = game_system.adnd.ADNDItem({"item_name": "Generic ADND Item One", "cost_money": 111})
...
class TestADNDItemStats(unittest.TestCase):
    def setUp(self):
        ...
        self.item_adnd_data = {"item_name": "Generic ADND Item One", "cost_money": 303}
    ...
    def test_costs_none(self):
        ...
        for tmp_cost in game_system.none.Item.cost_types:
            if tmp_cost in self.item_adnd_data:
                self.logger.debug("tmp_cost: %s, equip value: %s, data value: %s", tmp_cost, self.item_adnd.get_cost(tmp_cost), self.item_adnd_data[tmp_cost])
                self.assertEqual(self.item_adnd.get_cost(tmp_cost), self.item_adnd_data[tmp_cost])
            else:
                self.logger.debug("tmp_cost: %s, equip value: %s, 0", tmp_cost, self.item_adnd.get_cost(tmp_cost))
                self.assertEqual(self.item_adnd.get_cost(tmp_cost), 0)
    ...
    def test_mods_none(self):
        ...
        for tmp_mod in game_system.none.Item.mod_types:
            if tmp_mod in self.item_adnd_data:
                self.logger.debug("tmp_mod: %s, equip value: %s, data value: %s", tmp_mod, self.item_adnd.get_mod(tmp_mod), self.item_adnd_data[tmp_mod])
                self.assertEqual(self.item_adnd.get_mod(tmp_mod), self.item_adnd_data[tmp_mod])
            else:
                self.logger.debug("tmp_mod: %s, equip value: %s, 0", tmp_mod, self.item_adnd.get_mod(tmp_mod))
                self.assertEqual(self.item_adnd.get_mod(tmp_mod), 0)
